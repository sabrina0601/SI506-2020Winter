...
def format_event_location(event, location='building'):
    ...
    if location == 'city':
        ...
        return ', '.join(event[2:]) # default to building scope
        # return f"{event[2]}, {event[3]}, {event[4]}"
        # return '{}, {}, {}'.format(event[2], event[3], event[4])
...
def get_event_locations(tour_dates, location='building'):
    ...
    for event in tour_dates:
        place = format_event_location(event, location) # call function
...
def main():
    ...
    # Tour venues (building, city, country)
